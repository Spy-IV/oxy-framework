...
AddEventHandler('commandEntered', function(source, fullcommand)
	...
	if command[1] == "/help" or command[1] == "/commands" then TriggerClientEvent('chatMessage', source, '', { 0, 0x99, 255 }, "^6Commands: ^3/moddedvehicles | /veh ^7(vehicle name)^3 | /vehcol ^7(colour ID) (colour ID)^3 | /repair | /flip | /fix | /clean | /heal | /armour | /givewep ^7(weapon name)^3 | /suicide | /tp ^7(target player ID/name)^3 | /hudoff | /hudon")
...
----------------------------------------------------------------------------------------------------------------------------------------
	elseif(name == "admin" and command[1] == "/anim") then TriggerClientEvent('playAnimStuff', source, command[2], command[3], command[4], false, 0, 0, 0, 5000)

	elseif(name == "admin" and command[1] == "/checkpoint") then TriggerClientEvent('createCheckpoint', source)

	elseif(name == "admin" and command[1] == "/pos") then TriggerClientEvent('setPos', source, string.sub(command[2], 1, string.len(command[2])-1), string.sub(command[3], 1, string.len(command[3])-1), string.sub(command[4], 1, string.len(command[4])-1))

	elseif(name == "admin" and command[1] == "/audio") then TriggerClientEvent('playAudioStuff', source, command[2])

	elseif(name == "admin" and command[1] == "/giverpg") then TriggerClientEvent('giveWeapon', source, 18, 7)
	elseif(name == "admin" and command[1] == "/godmode") then TriggerClientEvent('godmode', source)
	elseif(name == "admin" and command[1] == "/savecar") then TriggerClientEvent('sendSaveCar', source)
	elseif(name == "admin" and command[1] == "/pos" or command[1] == "/savepos") then TriggerClientEvent('sendPos', source)

	elseif(name == "admin" and command[1] == "/kick") then
	...
	elseif(name == "admin" and command[1] == "/blip") then
	...
	elseif(name == "admin" and command[1] == "/weather") then
	...
	elseif(name == "admin" and command[1] == "/time") then
	...
	elseif(name == "admin" and command[1] == "/msgtoeveryone") then
	...
	elseif(name == "admin" and command[1] == "/chattoeveryone") then
